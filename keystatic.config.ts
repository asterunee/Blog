...
export default config({
  ...
  collections: {
    notices: collection({
      ...
      previewUrl: "/",
      format: "json",
      columns: ["title", "publishedAt", "priority", "visible"],
      schema: {
        title: fields.slug({ name: { label: "공지 제목", validation: { isRequired: true } }, slug: { label: "공지 파일 이름", description: "영문 소문자와 하이픈을 권장합니다." } }),
        summary: fields.text({ label: "공지 내용", multiline: true, description: "메인 배너에 표시할 짧은 내용을 입력하세요.", ...required }),
        label: fields.text({ label: "상단 표시", description: "예: 공지사항, 업데이트, 안내", defaultValue: "공지사항", ...required }),
        publishedAt: fields.date({ label: "게시일", defaultValue: today, ...required }),
        startsAt: fields.date({ label: "표시 시작일", description: "비우면 게시일부터 표시합니다." }),
        endsAt: fields.date({ label: "표시 종료일", description: "비우면 계속 표시합니다." }),
        href: fields.text({ label: "연결 주소", description: "비워도 됩니다. 내부 경로(/posts) 또는 전체 URL을 입력하세요." }),
        linkLabel: fields.text({ label: "버튼 문구", description: "연결 주소가 있을 때 표시됩니다.", defaultValue: "자세히 보기" }),
        newTab: fields.checkbox({ label: "새 탭에서 열기", defaultValue: false }),
        backgroundImage: fields.image({ label: "공지 배경 이미지", description: "비우면 사이트 기본 배경을 사용합니다.", directory: "public/images/notices", publicPath: "/images/notices/" }),
        ...
        priority: fields.integer({ label: "우선순위", description: "숫자가 큰 공지가 먼저 표시됩니다.", defaultValue: 0, validation: { isRequired: true, min: 0, max: 100 } }),
        visible: fields.checkbox({ label: "메인에 공개", description: "해제하면 저장해도 방문자에게 보이지 않습니다.", defaultValue: true }),
      },
    }),
