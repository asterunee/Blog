...
export default function ArchivePage() {
  ...
  return <div className="page-shell archive-index"><header className="page-title"><span className="section-index">ARCHIVE</span><h1>글 아카이브</h1><p>주제와 형식이 다른 모든 기록을 날짜별로 모았습니다.</p></header>{entries.length ? [...groups].map(([month, posts]) => <section key={month}><h2>{month.replace("-", " / ")}</h2><div>{posts.map((post) => <Link href={post.href} key={post.href}><time>{post.date.slice(8)}</time><span>{post.kind}</span><h3>{post.title}</h3></Link>)}</div></section>) : <div className="content-empty content-empty-page"><h2>아카이브가 비어 있습니다</h2><p>직접 작성한 글이 공개되면 날짜별로 자동 정리됩니다.</p><Link href="/keystatic">첫 기록 작성하기 →</Link></div>}</div>;
}
