...
export default async function CustomContentIndexPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <div className="page-shell archive-page">
    ...
    {entries.length ? <section className="category-entry-feed">{entries.map((entry) => <Link href={`/content/${section}/${entry.slug}`} key={entry.slug} prefetch={false}><div><span>{settings.label}</span><time>{entry.date}</time></div><h2>{entry.title}</h2><p>{entry.description}</p><footer><div>{entry.tags.slice(0, 5).map((tag) => <span key={tag}>#{tag}</span>)}</div><ArrowRight size={15} /></footer></Link>)}</section> : <section className="editorial-empty"><h2>아직 공개된 콘텐츠가 없습니다</h2><p>관리자 작성기에서 첫 콘텐츠를 공개하면 이곳에 표시됩니다.</p></section>}
  </div>;
}
