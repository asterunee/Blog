...
export default async function LogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="article-shell"><header className="article-header"><Link href="/log" className="back-link"><ArrowLeft size={15} /> 짧은 기록</Link><p>{post.type} · {post.date}</p><h1>{post.title}</h1><p className="article-description">{post.description}</p><div className="tag-row">{post.tags.map((tag) => <Link href={`/tags/${tag}`} key={tag}>{tag}</Link>)}</div><dl className="article-facts"><div><dt>Published</dt><dd>{post.date}</dd></div><div><dt>Updated</dt><dd>{post.updated}</dd></div><div><dt>Mood</dt><dd>{post.mood || "—"}</dd></div><div><dt>Location</dt><dd>{post.location || "—"}</dd></div></dl>{post.coverImage && <div className="article-cover"><Image src={post.coverImage} alt={post.coverAlt || ""} fill priority sizes="(max-width: 900px) 100vw, 820px" /></div>}</header><div className={`article-layout${headings.length ? "" : " without-toc"}`}>{headings.length > 0 && <TableOfContents headings={headings} />}<article className="prose"><MdxContent source={post.body} /></article></div></div>;
}
