...
export default async function AlgorithmPage({ params }: { params: Promise<{ algorithm: string }> }) {
  ...
  return <div className="page-shell archive-page">
    ...
    {entries.length ? <section className="category-entry-feed">{entries.map((entry) => <Link href={entry.href} key={entry.href} prefetch={false}><div><span>{entry.kind}</span><time>{entry.date}</time></div><h2>{entry.title}</h2><p>{entry.description}</p><footer><div>{entry.tags.slice(0, 5).map((tag) => <span key={tag}>#{tag}</span>)}</div><ArrowRight size={15} /></footer></Link>)}</section> : <section className="editorial-empty"><h2>아직 공개된 콘텐츠가 없습니다</h2><p>작성기에서 이 알고리즘 주제를 연결한 콘텐츠가 공개되면 자동으로 표시됩니다.</p></section>}
  </div>;
}
