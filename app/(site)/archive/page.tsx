...
export default function ArchivePage() {
  const entries = [...getPosts().map((post) => ({ ...post, href: `/posts/${post.slug}`, kind: getCategoryName(post.category) })), ...getSolutions().map((post) => ({ ...post, href: `/solutions/${post.slug}`, kind: "PS 풀이" })), ...getLogs().map((post) => ({ ...post, href: `/log/${post.slug}`, kind: post.type }))].sort((a,b) => b.date.localeCompare(a.date));
