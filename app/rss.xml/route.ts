...
export function GET() {
  const entries = [...getSolutions(false).map((post) => ({ ...post, href: `/solutions/${post.slug}` })), ...getLogs(false).map((post) => ({ ...post, href: `/log/${post.slug}` }))].sort((a,b) => b.date.localeCompare(a.date));
