...
export function GET() {
  const items = getSolutions(false).map((post) => `<item><title><![CDATA[${post.title}]]></title><link>${siteConfig.url}/solutions/${post.slug}</link><guid>${siteConfig.url}/solutions/${post.slug}</guid><pubDate>${new Date(post.date).toUTCString()}</pubDate><description><![CDATA[${post.description}]]></description></item>`).join("");
