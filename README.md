...
개발 서버 또는 배포 사이트의 `/keystatic`에서 일반 글, PS 풀이와 짧은 기록을 직접 작성할 수 있습니다. 로컬 개발에서는 저장 버튼이 `content/posts`, `content/solutions`, `content/log`의 MDX 파일을 바로 수정합니다.

일반 글 작성기는 자유 카테고리와 태그, 시리즈, 표지·본문 이미지 업로드, 글별 강조 색, SEO 제목·설명, canonical URL, 목차, 대표 글, 상단 고정과 초안 상태를 지원합니다. PS 풀이와 짧은 기록도 저지·언어·상태·실행 결과·기분·장소 등을 자유롭게 입력할 수 있습니다.

작성기의 `블로그 관리 → 사이트 설정`에서는 블로그명, 소개 문구, 현재 관심사, 배경 이미지·위치·강도·효과, 강조 색, 기본 테마와 PS 활동 위젯 표시 여부를 변경합니다. 저장한 설정은 Git에 기록되고 `main` 반영 후 자동 배포됩니다.
...
```env
NEXT_PUBLIC_SITE_URL=https://asterunee.vercel.app
...
- `content/posts`: 개발, 공부, 생각과 일상을 담는 일반 글 MDX
- `content/solutions`: Git으로 관리하는 PS 풀이 MDX
- `content/log`: 짧은 일지와 회고 MDX
