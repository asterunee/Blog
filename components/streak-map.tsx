...
export function StreakMap() {
  ...
  </div><div className="streak-legend"><span>적음</span>{[0,1,2,3,4].map((v) => <i key={v} className={`level-${v}`} />)}<span>많음</span></div><p className="microcopy">UTC 날짜별 제출 횟수입니다.{fallback && " 현재 저장된 활동 데이터를 표시합니다."}</p></div>;
}
