...
export function AllPostsExplorer({ entries }: { entries: UnifiedEntry[] }) {
  ...
  return <>
    ...
    {filtered.length ? <section className="unified-post-feed">{filtered.map((entry) => <article key={entry.href}><Link href={entry.href}><div className="unified-post-meta"><span>{entry.kind} · {entry.categoryName}</span><time>{entry.date}</time></div><h2>{entry.title}</h2><p>{entry.description}</p>{entry.tags.length > 0 && <div className="tag-row">{entry.tags.slice(0, 6).map((tag) => <span key={tag}>{tag}</span>)}</div>}</Link></article>)}</section> : <section className="editorial-empty"><h2>조건에 맞는 글이 없습니다</h2><p>검색어나 카테고리를 변경해 보세요.</p></section>}
  </>;
}
